...
const Grid = () => {
  ...
  const isW = useKeyPress("w");
  const isA = useKeyPress("a");
  const isS = useKeyPress("s");
  const isD = useKeyPress("d");
  const isPressUp = isArrowUp || isW;
  const isPressDown = isArrowDown || isS;
  const isPressLeft = isArrowLeft || isA;
  const isPressRight = isArrowRight || isD;
